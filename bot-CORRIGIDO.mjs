...
async function searchAllSources(query, mode = 'manhwa') {
  ...
  if (mode === 'anime') {
    // Busca em paralelo: AniList, Kitsu, Jikan, AniDB, tradução
    const [animeResults, translatedQuery, kitsuResults, jikanResults, anidbResults] = await Promise.all([
    ...
    if (merged.length < 3 && translatedQuery) {
      const [more, moreKitsu, moreJikan] = await Promise.all([
        searchAnime(translatedQuery).catch(() => []),
        searchKitsu(translatedQuery).catch(() => []),
        searchJikanAnime(translatedQuery).catch(() => []),
      ]);
      for (const r of more.map(anilistToUnified)) if (!isDuplicate(merged, r)) merged.push(r);
      for (const r of moreKitsu)  if (!isDuplicate(merged, r)) merged.push(r);
      for (const r of moreJikan)  if (!isDuplicate(merged, r)) merged.push(r);
    }

    if (merged.length < 3) {
      const noSpace = query.replace(/\s+/g, '');
      if (noSpace !== query && noSpace.length > 0) {
        const [r1, r2] = await Promise.all([
          searchAnime(noSpace).catch(() => []),
          searchKitsu(noSpace).catch(() => []),
        ]);
    ...
    // Busca em paralelo: AniList, MangaDex, Comick, MangaUpdates, Jikan, tradução
    const [manhwaResults, mangadexResults, comickResults, muResults, jikanResults, translatedForExa] = await Promise.all([
      ...
      isSynopsis ? translateToEnglish(query) : Promise.resolve(null),
    ]);
    ...
    if (merged.length < 3) {
      ...
      if (translated) {
        const [m1, m2, m3] = await Promise.all([
          (mode === 'all' ? searchManga(translated) : searchManhwa(translated)).catch(() => []),
          searchMangaDex(translated).catch(() => []),
          searchJikanManga(translated).catch(() => []),
        ]);
        ...
        for (const r of m3) if (!isDuplicate(merged, r)) merged.push(r);
      }
    }
    ...
    if (merged.length < 3) {
      ...
      if (noSpace !== query && noSpace.length > 0) {
        const [n1, n2] = await Promise.all([
          (mode === 'all' ? searchManga(noSpace) : searchManhwa(noSpace)).catch(() => []),
          searchMangaDex(noSpace).catch(() => []),
        ]);
        for (const r of n1.map(anilistToUnified))  if (!isDuplicate(merged, r)) merged.push(r);
        for (const r of n2.map(mangadexToUnified)) if (!isDuplicate(merged, r)) merged.push(r);
      }
    }
